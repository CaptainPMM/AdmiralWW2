	...
	public class Peer : IDisposable {
		...
		public void Dispose() {
			...
			try {
				...
				if (connected == 1) Listener.OnPeerDisconnect(this, null, DisconnectReason.Disposed, null);
			} catch { }
			...
			try { Host.Allocator.SequenceReturn(ref SequenceReceive); } catch { }
			try { Host.Allocator.SequenceReturn(ref SequenceSend); } catch { }
			try { Host.Allocator.SequenceReturn(ref SequenceUnsequenced); } catch { }
		...
		internal void Connect(IWritable message = null) {
			...
			try {
				...
				lock (ConnectLock) {
					...
					Host.Allocator.ReturnPacket(ref ConnectRandom);
					if (Config.RemotePublicKey != null) {
					...
					int lengthRandom = Config.RemotePublicKey != null ? Host.Authenticator.SignatureLength : 0;
			...
			try {
				...
				if (state == 1) {
					...
					await SendReliableAsync(new MessageSent(
						...
						(ushort)Interlocked.Increment(ref SequenceSend[ChannelDisconnect]),
						ChannelDisconnect
					));
				...
				// Peer has been disposed during delay, ignore
				Listener.OnPeerDisconnect(this, null, DisconnectReason.Disconnected, null);
				Dispose();
		...
		public MessageSent Send(IMessage message, IMessageListener listener = null) {
			...
			if (sequenced) Interlocked.Exchange(ref SequenceUnsequenced[channel], 0);
			...
			while (true) {
				...
				try {
					await SendReliableAsync(new MessageSent(
						...
						(ushort)Interlocked.Increment(ref SequenceSend[ChannelPinger]),
						ChannelPinger
					));
				} catch (Exception exception) {
					Listener.OnPeerException(this, exception);
		...
		private Task SendFlushAsync(MessageSent message, byte attempt) {
			lock (FlushLock) {
				...
				try {
					...
					if (message.Flags.HasFlag(MessageFlags.Timed)) FlushWriter.Write((ushort)message.TimeCreated.Ticks);
		...
		private async Task FlushAsync() {
			...
			try {
				...
				// Encrypt
				if (Host.Config.Encryption) {
					int max = ConnectEncryptor.MaxEncryptedLength(packet.Count) + packet.Offset;
					bufferEncrypt = Host.Allocator.CreateMessage(max);
					int len = ConnectEncryptor.Encrypt(packet, bufferEncrypt, packet.Offset);
					...
					lock (FragmentLock) {
						...
						try {
							...
							if (FragmentReceived == null) {
								...
								Array.Clear(FragmentReceived, 0, receivedLength);
							}
		...
		private void OnReceiveAccept(ArraySegment<byte> packet) {
			...
			} else if (lengthSignature > 0 && Config.RemotePublicKey == null) {
				throw new FormatException(string.Format(
		...
		private async Task OnReceiveConnected(ArraySegment<byte> message, ushort? sent) {
			...
			if (flags.HasFlag(MessageFlags.Unique) && type != MessageType.Acknowledge && Config.DuplicateTimeout > 0) {
				...
				lock (UniqueLock) unique = UniqueReceived.Add(tuple);
				if (unique) {
					_ = Task.Factory.StartNew(
		...
		private async Task<bool> DelayAsync(int delay, CancellationTokenSource token) {
			...
			try {
				...
				return token.IsCancellationRequested;
			} catch (ObjectDisposedException) when (DisposeToken.IsCancellationRequested) {
				throw new ObjectDisposedException(GetType().FullName);
			} catch (TaskCanceledException) when (DisposeToken.IsCancellationRequested) {
				throw new ObjectDisposedException(GetType().FullName);
